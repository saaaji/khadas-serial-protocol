...
constexpr int READ_BUFFER_MAX_SIZE = 4096; // max size (in bytes) of the read buffer
...
struct RawPacket {
    ...
    /// @brief default constructor for packet draft
    /// @return empty packet draft
    RawPacket() {}
};
...
class SerialRequestQueue {
    ...
    public:
        ...
        bool deq_request(RawPacket& dst) {
            if (empty())
                return false;
